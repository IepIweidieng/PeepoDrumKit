		...
		else
		{
			...
			for (const auto& it : list)
			{
				...
				Gui::DisableFontPixelSnap(true);
				{
					...
					if constexpr (std::is_same_v<T, SuddenChange>) { text = std::string_view(b, sprintf_s(b, "%gs/%gs", it.AppearanceOffset, it.MovementOffset)); }
		...
		static constexpr auto itemsToClipboardText = [](const std::vector<GenericListStructWithType>& inItems, Beat baseBeat) -> std::string
		{
			...
			for (const auto& item : inItems)
			{
				...
				case GenericList::Sudden:
				{
					...
					bufferLength = sprintf_s(buffer, "Sudden { %d, %g, %g };\n", (in.BeatTime - baseBeat).Ticks, in.AppearanceOffset, in.MovementOffset);
				} break;
